...
**Access:** arXiv API (`http://export.arxiv.org/api/query`) — a distinct endpoint, query syntax, and response schema (Atom XML) from the primary corpus's Zenodo REST API (§2.1, JSON). This is a separate extraction/parsing implementation, not shared code with the Zenodo pipeline, and is **not yet built as of v1.3** (§8, item 12) — the first live pilot batch (§ pilot note, v1.3) ran against Zenodo only.
...
**Access:** Same arXiv API as §2.2, not yet built as of v1.3.
...
1. Run the inclusion-criteria query against each corpus per year-bin; record the total matching count per bin — call this **N_year**. This alone is a reportable deliverable: a raw growth curve for the primary corpus, independent of classification, and it is also the denominator used by the volume estimate in §6.5. **Each record pulled must capture, at minimum: record ID, upload/publication date (for versioned Zenodo records, the earliest version's date, with the full version-date history captured separately for §5's outflow measurement), and the query term matched.** (v1.3) The first live pilot batch run against this protocol (July 2026, 37 unique records across two query terms) did not capture a date field. This made the batch usable for §3 classification and prevalence counting but left it unable to support §2.4's year-bin assignment or any part of §6.3/§6.5 (both depend on knowing which year-bin each record belongs to). Date cannot be reliably reconstructed after the fact from record ID alone (Zenodo IDs are not date-ordered in a documented way) — it must be captured at collection time.
...
No single marker is sufficient. A document is classified as an **elaboration-drift specimen** only if it satisfies the mandatory markers (M1, M3-manual) plus at least one supporting marker (M4 or M5), with M2 and M3-manual both confirmed by manual review. M2 and M3-manual are the load-bearing evidentiary markers, assessed by the same two coders in the same review pass on the stratified sample per §2.4 — neither is a full-corpus filter. (v1.3: M3 was split into an automated triage signal, M3-auto, and a mandatory manual judgment, M3-manual, after the first live pilot batch showed the automated phrase-list form of M3 does not generalize — see M3 below.)

### M1 — Formal-Scaffolding Density (automatable pre-filter)
Presence of explicit "Theorem," "Lemma," "Corollary," or "Proof" labels, at a density (labels per 1,000 words) exceeding the 90th percentile density observed in a reference sample of accepted journal articles in the same nominal subfield. High formal-apparatus density alone is not evidence of anything; it is a pre-filter that bounds the set requiring manual review. As of v1.3, M1 is the **sole automated gate** into manual review (§4) — see M3 below for why M3 no longer co-gates.
...
### M3 — Field-Encompassing Framing (split: automated triage signal + manual required judgment; revised v1.3)
Presence of rhetoric asserting the result is field-ending, industry- or discipline-wide, immune to standard objections, or reversing the burden of proof onto an entire field rather than onto a specific prior claim — **or**, a broader form the phrase-list below does not catch, a claim to have simultaneously resolved multiple independently-famous open problems in a field (e.g., "resolves the black-hole information paradox, the Collatz conjecture, the twin-prime conjecture, and Gödel incompleteness from a single premise") stated in a register with no confrontational vocabulary at all.

**M3-auto (automated triage signal; weak; never a gate).** A seed phrase-pattern list (built the same way as the Noether protocol's immune-vocabulary pool, §3.2 of that protocol: empirically derived from a training subsample, then validated against a hand-coded seed list at >60% Jaccard overlap), not a single fixed string match. Short/generic fragments in this list (e.g., "the only move," "checkmate") require an anchor word from a fixed list (cannot, absorb, immune, theorem, proof, argument, …) within a character window of the match, since bare fragments false-positive on ordinary usage otherwise — confirmed empirically in the pipeline-mechanics pilot run of `tools/elaboration_drift_pipeline.py` (private repo): of 46 raw contextual-phrase occurrences in a test corpus, 26 were correctly suppressed as unrelated casual usage once the anchor-window requirement was applied. **Demoted from mandatory automated gate to weak triage signal in v1.3**: the first live pilot batch (37 unique Zenodo records, two query terms) registered **zero** M3-auto hits across the entire sample, including both records independently hand-confirmed to exhibit extreme field-encompassing framing — one claiming simultaneous resolution of the black-hole information paradox, the Collatz and twin-prime conjectures, Gödel incompleteness, and four further open problems from the single premise "A = A." The phrase list was derived from the two original case-study specimens' specific confrontational vocabulary and does not generalize to the grandiose-unification register these pilot specimens use instead. M3-auto retains value as a cheap, reportable triage signal for the register it was built from (alongside M5 in the output, §10.1 item 8), but must never gate advancement to manual review and never by itself satisfies the mandatory-marker requirement for a confirmed specimen.

**M3-manual (mandatory; manual; load-bearing).** For every document advancing to manual review (§4), the same two independent coders who perform M2 render a structured 0/1 judgment on field-encompassing framing: does the document claim, explicitly or as an unstated background premise, that its result resolves, dissolves, or subsumes an entire field, discipline, or multiple independently-famous open problems, rather than making a scoped, checkable claim about a specific narrower question? Blind to the other coder's judgment, with inter-rater reliability reported using the same Cohen's kappa bar as M2 (§9, item 3). This is M3's load-bearing form — a document satisfies M3 for classification purposes (this section's opening paragraph, §4 step 5) only via M3-manual confirmation, never via M3-auto alone.
...
1. Run M1 as the automated pre-filter across the full sampled batch from §2.4 (density exceeding the reference-sample threshold, §3 M1). Record M3-auto's phrase-list score for every document for descriptive/triage purposes only — **do not gate on it** (v1.3: see §3 M3 for why the earlier M1-AND-M3-auto gate is retired — the first live pilot batch found it passed zero of 37 records, including both hand-confirmed true positives, because M3-auto's phrase list registered no hits at all in that batch).
2. For documents passing the M1 pre-filter, compute M4 and M5 from metadata.
3. Documents satisfying M1 + (M4 or M5) advance to manual review.
4. Two independent coders evaluate M2 (citation-support mismatch) **and M3-manual (field-encompassing framing)** per document in the same review pass, blind to each other's judgment and to the document's M4/M5 status. Report inter-rater reliability (Cohen's kappa) separately for M2 and M3-manual; disagreements on either resolved by a third coder, per the same reliability bar as the Noether protocol (§9.3 there, κ > 0.80).
5. A document is a confirmed **elaboration-drift specimen** only if it passes step 3 and both M2 and M3-manual are independently confirmed by the coders (or the arbiter) for that document.
...
10. **Abstract/metadata-only scope, confirmed empirically (v1.3).** The first live pilot batch (37 unique Zenodo records, two query terms, July 2026) drew its text from record metadata, not full deposit body text. 12 of 37 records were under 130 words — several (17, 30, 33, 36, 37 words) are barely more than a title. One confirmed true positive ("The Totality Theorem," hundreds of PDF pages) scored a 277-word metadata description; its field-encompassing claims and formal apparatus are documented deep in the PDF body, invisible to any metadata-only pass. M1/M3 scores computed from metadata alone should be read as a lower bound on the true density/framing signal, not a full reading. Full-text/PDF-body extraction is required future work and was not implemented for the first pilot.
11. **M3-auto's original phrase-list register does not generalize (v1.3).** See §3 M3 for the fix (M3-manual). Any prevalence figures computed before this fix — i.e., under the retired M1-AND-M3-auto gate — would have undercounted toward zero regardless of true prevalence, since M3-auto's zero-hit rate in the pilot batch made the gate unsatisfiable. No such figures have been published from this protocol; this item exists so the retired gate is never silently re-adopted (e.g., by a future implementation that re-derives §4 from a stale version of this document).
12. **arXiv corpus access not yet implemented (v1.3).** §2.2 and §2.3 depend on the arXiv API, a distinct endpoint and response schema from the Zenodo REST API used in §2.1. The first live pilot ran against Zenodo only; H1b (Zenodo-specificity, §6.2) and the moderated-control comparison (§6.1) cannot be tested until the arXiv extraction path is built.
...
4. **Code and seed lists.** The M3-auto phrase-pattern derivation code, the M1 density-threshold reference sample, and all classification code must be published with fixed random seeds.
5. **Per-record date (v1.3).** The upload/publication date, and where applicable the full version-date history, for every sampled record must be published alongside the record ID, per the §2.4 v1.3 addition. A sample list without dates cannot support §2.4's year-bin assignment and must be labeled as unable to support §6.3/§6.5 if published anyway (as with the first pilot batch, §8 item 10 context).
...
8. Full classification audit trail: sampled record list (with dates, §9 item 5), M1/M3-auto/M4/M5 scores, M2 and M3-manual coder judgments and kappa (both reported), M6 recorded separately.
...
*v1.3 (July 2026): three fixes driven directly by the first live pilot batch (37 unique Zenodo records, two query terms — "unified theory," "quantum mechanics" — run July 2026, reported in full). (1) **Pre-filter gate fix**: the M1-AND-M3-auto automated gate (§4 old step 1) passed 0 of 37 records, including both records independently hand-confirmed as true positives, because M3-auto's phrase list never fired in this batch. M1 is now the sole automated gate (§4); M3 is split into M3-auto (automated, weak triage signal, never a gate) and M3-manual (mandatory, manual, load-bearing — assessed by the M2 coders in the same review pass, §3 M3, §4). (2) **Missing date field**: the pilot batch did not capture record upload dates, leaving it unable to support §2.4 year-binning or §6.3/§6.5; date is now an explicit required field at collection time (§2.4, §9 item 5). (3) **arXiv access route flagged as unbuilt**: §2.2/§2.3 depend on a distinct arXiv API implementation not yet written; the pilot ran Zenodo only (§8 item 12). No prevalence, trend, or volume statistic has been published from this protocol at any version; all three fixes were made before any such figure was reported.*
