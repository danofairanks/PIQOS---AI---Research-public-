"""Command-line interface: `rigor-cosplay text|corpus`."""
...
def _cmd_text(args: argparse.Namespace) -> int:
    if args.file:
        ...
        text = sys.stdin.read()
    result = scan(text, radius=args.radius)
...
def _cmd_corpus(args: argparse.Namespace) -> int:
    ...
    summary = scan_corpus(documents, radius=args.radius)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="rigor-cosplay",
        description="Classifier for Rigor Cosplay: cosmetic pushback, weak-man steelmanning, "
        "and honesty-as-flattery",
    )
    ...
    p_text.add_argument(
        "--radius", type=int, default=DEFAULT_PROXIMITY_RADIUS,
        help="character-distance co-occurrence window (default: %(default)s)",
    )
    ...
    p_corpus.add_argument("--radius", type=int, default=DEFAULT_PROXIMITY_RADIUS)
