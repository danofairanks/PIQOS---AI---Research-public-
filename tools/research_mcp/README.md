...
| `bifp_start_closed_path_ledger` | bifp | Create a new closed-path evidence ledger for one artifact (`closed_path_confirmation_v1.md` §2) |
| `bifp_record_fixture` | bifp | Classify one fixture as asserted/derived/unknown |
...
| `attractor_scan_claim_boundary_portability` | attractor_scan | Two-document check: does a source's own stated scope limitations show any lexical trace in a separate citation/reference text? |
