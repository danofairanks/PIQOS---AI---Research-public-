...
**The same input:output gap, one layer up — the review pipeline itself, not just individual proofs.** AAAI's 2026 cycle received 30,948 initial submissions, roughly double the prior cycle's volume, falling to approximately 23,680 after policy-compliance filtering (17.6% acceptance rate, 4,167 accepted) — a rate of growth partly attributed to AI-assisted paper generation. ICLR responded for its 2027 cycle with explicit quotas rather than absorbing the volume: no author may appear on more than 20 co-authored submissions, and any author listed on three or more submissions must serve as a reviewer for at least six, with a one-paper cap for author teams with no prior top-tier publication record (ICLR 2027 Author Guidelines, 2026). This is the review-capacity version of the Leiden Declaration's concern, not a separate phenomenon: AI-accelerated output growth — submissions, not proofs — outpacing the human capacity the field relies on to check it, with the institution visibly throttling the pipeline by structural rule rather than narrative reassurance. Both responses share the same shape this paper argues for throughout: verification infrastructure has to scale with output, or the field has to constrain output to match verification capacity — there is no third option where volume grows unchecked and quality holds.
...
**Both categories share a ceiling this paper's own verification independently locates.** Neither the search-problem five nor the general-regularity five originates a new axiom, resolves a foundational inconsistency between two existing frameworks, or reframes a domain — every one of the ten works inside an already-established mathematical structure, as Chapter 9 states of its own method directly: "The saturated-matrix construction is not new; its application to R(3,...,3) is." This matches, independently and prior to this paper encountering it, the specific ceiling a contemporaneous DeepMind position paper argues LLMs cannot cross: Tom Zahavy's "LLMs can't jump" (ICML 2026) distinguishes deduction and induction — both now performed well by frontier models — from abduction, the origination of a new explanatory premise, illustrated by Einstein's move from Newtonian mechanics to general relativity rather than a better fit to noisier data. Full treatment, including convergence with Gary Marcus's and Tim Gowers's independently-verified assessments of the Erdős #1196 result on the same axis: [`../../case_studies/2026-08-02_ten_advances_abductive_jump_test.md`](../../case_studies/2026-08-02_ten_advances_abductive_jump_test.md).
...
**A second self-interest-stated-directly instance, this time about knowledge rather than money.** Microsoft CEO Satya Nadella made a substantively valid, independently verifiable critique of the industry's data economics: "While the great innovation that comes from model providers having fair use rights to train models on public data is needed, I find it ironic that the status quo is to then turn around and impose restrictive terms on distillation, and to reserve the right to learn from customer usage and interaction data" (Satya Nadella, X, July 12, 2026). The double standard he names is real: labs claim fair use to train on the open internet, then write restrictive terms preventing anyone from learning off their own outputs. His prescribed fix — enterprises should own their data, build learning environments in the cloud, use model-agnostic orchestration layers — points toward Azure, stated plainly enough that the self-interest is not hidden.

The check this section already applies to Ho's testimony above applies here too, and it does not come out clean. Microsoft's own Generative AI Services terms explicitly prohibit customers from using Azure OpenAI or Copilot outputs "for the express purpose of creating synthetic training data to develop or train AI models or systems that have substantially similar functionality to a Microsoft AI service" — a distillation restriction, the same shape Nadella calls ironic when other labs impose it. Separately, GitHub Copilot — a Microsoft product — remains a defendant in *Doe v. GitHub*, an unresolved lawsuit (9th Circuit appeal pending, oral arguments February 2026) alleging Copilot trained on billions of lines of licensed open-source code without complying with the attached license terms: the same "fair use to train on everyone else's material" pattern the critique targets, with Microsoft's own exposure here still legally open rather than settled either way (contrast §2.11's *Bartz v. Anthropic*, which reached final settlement). One specific claim does not hold against Microsoft: Nadella's objection to labs that "reserve the right to learn from customer usage and interaction data" is not mirrored in Microsoft's own stated policy, which asserts customer data is not used to train foundation models without explicit customer permission.

Critique does not earn immunity from the standard it invokes. Nadella's underlying point about the industry's data economics stands on its own evidence regardless of who makes it; that the maker is simultaneously subject to two of the three practices named and exempt from the third is the same pattern this section documents throughout — a genuine claim and a self-interested position occupying the same statement, distinguishable only by checking each specific piece against the record rather than accepting or dismissing the whole on the speaker's identity.
