...
A single trajectory in which R is maximized and C is violated defeats
this claim. That is the entire method: state the claim as a universal,
then try to construct or find the trajectory that breaks it.
...
- Altman, E. (1999). *Constrained Markov Decision Processes.* CRC
  Press. — the base formalism for hard vs. soft (Lagrangian-penalty)
  constraint enforcement under an optimizing policy.
- Amodei, D., Olah, C., Steinhardt, J., Christiano, P., Schulman, J., &
  Mané, D. (2016). *Concrete Problems in AI Safety.* arXiv:1606.06565.
  — the reward-hacking / specification-gaming framing §6.3 draws the
  precise distinction from.
- Krakovna, V., et al. *Specification gaming: the flip side of AI
