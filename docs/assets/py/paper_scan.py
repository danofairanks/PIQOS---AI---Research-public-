...
the same seven agent_tools.py surfaces tools/research_mcp/ wraps for
MCP -- no new research logic, just calling the seven text->dict
...
Deliberately does NOT compute a single blended "rigor score." The
seven tools measure independent, differently-scoped things -- a self-
citation ratio is not commensurable with a semantic-laundering match
count. Inventing a composite number would be exactly the kind of
unearned precision this project's own papers (laundered_vocabulary_v1.md's
...
def run_all_scans(text: str, *, title: str = "") -> dict:
    """Run all seven scanners against `text` and return one combined,
...
def _md_rigor_cosplay(section: dict) -> list[str]:
    lines = ["### rigor_cosplay", ""]
    hits = section["signatures_hit"]
    if hits:
        lines.append(f"- Signatures flagged: {', '.join(hits)}")
        if "cosmetic_pushback" in hits:
            lines.append(f"  - {len(section['cosmetic_pushback'])} cosmetic-pushback pair(s) "
                         "(praise + a pushback-announcement close together)")
        if "weak_man_steelman" in hits:
            lines.append(f"  - {len(section['weak_man_steelman'])} steelman-announcement match(es) "
                         "(presence only -- whether the steelman itself is weak needs reading)")
        if "honesty_as_flattery" in hits:
            lines.append(f"  - {len(section['honesty_as_flattery'])} honesty-marker + praise pair(s)")
    else:
        lines.append("- No Rigor Cosplay signatures flagged.")
    lines.append("")
    return lines


def _md_hedge_dogwhistle(section: dict) -> list[str]:
    lines = ["### hedge_dogwhistle", ""]
    lines.append(f"- Paralipsis construction(s) found: **{_yn(section['has_paralipsis'])}** "
                 f"({len(section['matches'])} match(es))")
    if section["has_paralipsis"]:
        lines.append("  - Text with each disclaiming sentence removed (re-read this and judge "
                     "whether the disclaimed content's association survives without the hedge):")
        lines.append(f"    > {section['text_with_hedges_removed']}")
    lines.append("")
    return lines
...
def generate_markdown_report(combined: dict) -> str:
    ...
    lines.append("Ran entirely in your browser via Pyodide -- the scanned text was never sent "
                 ...
                 "`rigor_cosplay`, `hedge_dogwhistle`), "
    ...
    lines.extend(_md_rigor_cosplay(combined["rigor_cosplay"]))
    lines.extend(_md_hedge_dogwhistle(combined["hedge_dogwhistle"]))
    lines.append("## What this report does NOT establish")
    lines.append("")
    lines.append("- **No blended score.** These seven checks measure independent, "
