...
| [`tools/rigor_cosplay/`](tools/rigor_cosplay/) | `pip install`-able classifier for **Rigor Cosplay** — a response that adopts the surface markers of critical engagement (announcing pushback, offering to steelman an opposing view, declaring an intent to be honest) while the function of that engagement is inverted: the pushback lands on a peripheral detail, the steelman is weak, the "honesty" is praise. Three independent signature detectors, any one of which is sufficient on its own — unlike `debasinizer`'s resonance register, they do not need to co-occur with each other. Explicit about what a phrase scanner cannot judge: it cannot tell a genuinely peripheral pushback from a load-bearing one, or a weak steelman from a strong one — both always flag as leads, never verdicts. Distinguished directly from the adjacent retreat-under-pressure mechanism (goalpost movement under sustained challenge, a different, already-tracked pattern) — this one is spontaneous and unprompted. 21 tests, all passing, including a proximity-gate negative control (praise and a pushback phrase present in the same long response but far enough apart not to co-occur) and a clean substantive-pushback negative control. |
| [`tools/hedge_dogwhistle/`](tools/hedge_dogwhistle/) | `pip install`-able detector for **paralipsis** ("I'm not saying X, but...") plus a mechanized first step of a falsifiable removal test: delete every sentence containing the construction and return what remains, so a human or agent can judge whether the disclaimed content's association survives — established elsewhere, unhedged (ordinary scope-limiting hedge), or gone entirely (consistent with the content arriving only through the denial). Explicit that this mechanizes the removal step only, not the semantic judgment the test is actually for — a real specimen check using this same test in this project's own research notes returned a negative result for exactly the reason the tool is built to surface (the disclaimed content was already stated directly, unhedged, two sentences earlier). Deliberately does not detect insinuation-without-denial phrasing ("make of that what you will") — a related but distinct rhetorical move. 26 tests, all passing, including both a constructed positive-shaped and negative-shaped removal outcome. |
